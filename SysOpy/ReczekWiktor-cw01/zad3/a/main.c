...
void writeResultToTerminalAndFile(clock_t start, clock_t end, struct tms* t_start, struct tms* t_end){
...
int main(int argc, char **argv) {
    int i;
    int size = argc;
    resultFile = fopen("./results3b.txt", "a");
    struct tms * tms[size];
    clock_t time[size];
    for(i = 0; i < size; i++){
    ...
    for(i=2; i<argc; ++i){
        ...
        if(strcmp(argv[i], "create_table") == 0){
            ...
            error("Bad argument");
        time[current] = times(tms[current]);
        writeResultToTerminalAndFile(time[current-1], time[current],
